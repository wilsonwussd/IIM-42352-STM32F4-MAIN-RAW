...
**根本原因：**
- 传感器从LP模式（50Hz）切换到LN模式（1000Hz）后，前200-500ms数据有缓慢DC漂移（<1Hz）
- 数据特征：从0.51缓慢下降到-0.05，或从-0.60缓慢上升到0.00（单调变化）
...
**解决方案演进：**
- ✅ 第一版：增加LN模式启动后等待时间（50ms → 200ms）
- ✅ 第一版：检测开始前额外等待100ms
- ✅ 第一版：总稳定时间300ms，成功率80%
- ✅ 第二版：进一步增加稳定时间（200ms → 300ms，100ms → 200ms）
- ✅ 第二版：总稳定时间500ms，预期成功率95%+

**测试结果（5个案例）：**
```
✅ 案例1：挖掘7.81Hz，置信度0.9508 - 成功
❌ 案例2：DC漂移，FFT全0 - 失败（触发优化）
✅ 案例3：无振动，快速返回 - 成功
✅ 案例4：挖掘9.77Hz，置信度0.9302 - 成功
✅ 案例5：挖掘7.81Hz，置信度0.8814 - 成功（边界情况）
